        ...
        for (var i = 0; i < MaxToolCallsPerPhase; i++)
        {
            ...
            switch (normalizedAction)
            {
                case "get_workflow_input":
                {
                    ...
                    await logs.AppendLineAsync(workflowRunId, $"Tool call ({phase.Name}): get_workflow_input('{requestedWorkflowRunId}').", ct);
                    await logs.AppendBlockAsync(workflowRunId, $"Workflow input payload ({phase.Name})", inputPayload.InputPayloadJson, ct);

                    AppendToolInteraction(transcript, rawText, $"TOOL RESULT FOR get_workflow_input('{requestedWorkflowRunId}'):",
                    [
